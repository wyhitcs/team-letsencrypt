...
The most important component of letsencrypt is plugin, there are 4 major kinds of plugin: User Interface (UI), Configurator, Authenticator, Installer. The leader of this project use interfaces to standardlize the contribution of volunteers (contributors).
The interfaces available for plugins to implement are defined in [interfaces.py](https://github.com/letsencrypt/letsencrypt/blob/master/letsencrypt/interfaces.py) and [plugins/common.py](https://github.com/letsencrypt/letsencrypt/blob/master/letsencrypt/plugins/common.py#L34). The plugins must implement one of the four interfaces:
 ...
 - **Configurator**:  This component is used to automatically change configuration for different purpose, for example, if you want to                       change the folder to store your certificate, you need to do some configuration changes, an corresponding                              configurator can help you do so.  You may need different configurator for different use, let'encrypt allows                           contributors write their own contributors, but those contributors must inplement the interface of contributor.

 - **IDisplay**:  Implement bindings to alternative UI libraries. UI is a kind of plugin in let's encrypt, let's encrypt allows                         contributors to write their own UI, but those UI must implement IDisplay

 - **Authenticators**: Prove client deserves a certificate for some domain name by solving challenges received from the ACME server.                         For different types of challenges (e.g. simple HTTP challenge, DVSNI challenge), you need different                                   Authenticators. Let's encrypt allow contributors to construct new authenticators, but all uthenticators must                          implement Authenticators Interfaces

 - **Installer**: Setup the certificate in a server, possibly tweak the security configuration to make it more correct and secure. For                   different Web Server you use in your machine, you should use different Installer. You can construct your own                          Installers, but they must implement Installer Interface 
 
 
##Standardlization of Testing
There are two kinds of tests: Unit Test and Intergration Test.
 - **Unit Test**: 
 The Unit Test in let's encrypt to aim to check whether the code style compatiable with the required style, there are two files related to Unit Test: pep8 and tox.ini. The Test can be run off-line by developers.

- **Integration Test**: 
Integration Test is testing whether let's encrypt works well with boulder (the software run in Certificate Authority which generate SSL/TLS for you), this test is online test which will be run automatically after you do a pull request. The module corresponding for Intergration test is Travis CI.
